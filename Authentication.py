import csv
import Checking_functions as chf
...
def sign_up():
    ...
    def user_name_provider():
        ...
        if chf.user_exists(username) is True:
            print('User name already exists. Provide a different one: ')
            user_name_provider()
        if chf.char_num_user(username) is False:
    ...
    def password_provider():
        ...
        if chf.char_num_password(password) is False:
...
def log_in():
    ...
    user_name_verification = chf.user_exists(user_name)
    password_verification = chf.password_correct(password)
